#*******************************************************************************
#  File: XFintoARA.py
#     This script translates XF output into readable AraSim input
#
#  Programmer: Jorge Torres
#              Suren Gourapura
#
#  Revision history:
#     11/??/17 Original version
#     02/21/19 Suren, modified from the old XFintoARA.py
#     07/19/23 Jason Yao, merged XFintoARA2.py into this script because
#              the latter contains no significant difference (see line 87)
#
#  Notes:
...
#*******************************************************************************
'''
Have all XF output files in the same folder (see uanLoc) with the standard name:
    "antennaNumber_frequencyNumber.uan"
Both of these numbers start counting on 1, not zero.

Will output one file (see datLoc) that combines all frequencies,
so one dat file per antenna. 
The dat standard name is: 
    "evol_antenna_model_(i).dat" 
where (i) is the antenna number starting from 1, not zero.

Code Headers:		
  .uan header: Theta, Phi, Gain Theta (dB), Gain Phi (dB), Phase Theta, Phase Phi
...
### IMPORTS ###
import numpy as np  # For array storage, reading and writing files, etc.
import argparse     # For reading arguments from the terminal or from the bash script
import os           # For changing the permissions


### GLOBALS ###
# CORRECTED  Frequency List in MHz
freqVals = [ 83.33, 100.00, 116.67, 133.33, 150.00, 166.67, 183.34, 200.00,
...
# location on machtay1 "/users/PAS0654/machtay1/BiconeEvolution/current_antenna_evo_build/"\
#                      "XF_Loop/Evolutionary_Loop/Antenna_Performance_Metric/"
uanLoc = "/fs/ess/PAS1960/BiconeEvolutionOSC/BiconeEvolution"\
...
def readFile(indiv, freqNum):
  ...
  # changed from g.indiv + 1 to just indiv 

  # The following (commented out) uanName is the only part that XFintoARA2.py
  # differs from this script.
  # uanName = \
  ...
  # ie. m rows n cols // Jason 0612,23
  ...
  for i in range(n):
    for j in range(m):
      ...
      # %'s are for interpolation (2 digits after decimal point)
      ...
      lineFinal = lineList[0] + lineList[1] + \
                  lineList[2] + linearGain + lineList[4]+"\n"
...
parser.add_argument("NPOP",
                     help="How many antennas are being simulated each generation?",
                     type=int)
...
parser.add_argument("RunName",
                    help="Run Name directory where everything will be stored",
                    type=str)
